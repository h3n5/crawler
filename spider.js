...
;(async function crawler_wei_bo() {
  const url = "https://s.weibo.com/top/summary?cate=realtimehot"
  let html = await http.get(url)
  
  let host = html.request._header.match(/Host:.*\b/)[0].split(':')[1].trim()
  if (html.data) {
...
;(async function crawler_dou_ban() {
...
;(async function crawler_tian_ya() {
